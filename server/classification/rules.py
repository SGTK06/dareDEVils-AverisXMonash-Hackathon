...
LEXICAL_RULES = {
    ...
    "GENERAL": {
        ...
        "no action required": 2.5,
        "office resumes normal operations": 3.0,
        "reminder": 1.5,
...
def lexical_scores(text: str) -> dict[str, float]:
    ...
    for category, rules in LEXICAL_RULES.items():
        scores[category] += sum(weight for phrase, weight in rules.items() if phrase in lowered)
